#!/usr/bin/env python
# -*- coding: utf-8 -*-
# APLIB Compression and Decompression taken from Ange Albertini's kabopan library http://code.google.com/p/kabopan/
# aPLib is a LZSS based lossless compression algorithm by Jorgen Ibsen U{http://www.ibsensoftware.com}
# Modifications include removal of unused variables and indentation to conform to PEP8
...
class DTAplib(DTOperation):
    '''Perform APLIB compression and Decompression'''

    def needs_key(self):
        '''APLib does not need first key'''
        return False

    def needs_second_key(self):
        '''APLib does not need second key'''
        return False

    def needs_IV(self):
        '''APLib does not need IV'''
        return False

    def transform(self, dt_input, dt_key1=None, dt_key2=None, dt_iv=None, dt_mode='dec'):
        '''Return input as output.'''
        if dt_mode == 'dec':
            return decompress(str(dt_input)).do()[0]
        else:
            return compress(str(dt_input)).do()
